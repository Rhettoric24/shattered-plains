...
export const RESEARCH_RULES = {
  ...
  libraries: {
    economic: { name: "Economic Studies", description: "Ardents pursue the subtle crafts of markets, construction, and warcamp economics." },
  ...
  projects: {
    ...
    soulcasting: { name: "Warcamp Architecture", library: "economic", description: "Better camp plans, construction methods, and material discipline reduce waste across every major building project.", ancient: [0, 0, 0], gemhearts: [0, 0, 0], monastery: [1, 2, 3], costs: [2000, 5000, 7000], durationsMs: [3600000, 14400000, 43200000], effects: [5, 10, 20], effect: "% total building discount" },
...
export const PLATEAU_RUN_RULES = {
  ...
  sphereRewardPerActivePlayer: 3000,
...
export function effectivePower(units: Partial<UnitCounts>, completed?: Record<string, number>, conclaveCombat = false) {
  const normalized = normalizeUnits(units);
  const beforeConclave = basePower(normalized) + normalized.spearman * Number(researchEffect(completed, "soulcastArmor")) + shardbearerBreakthroughBonus(normalized);
  if (!conclaveCombat || researchLevel(completed, "religiousStudies") < 3) return beforeConclave;
