...
export default defineSchema({
  ...
  playerResearch: defineTable({
    ...
    activeDurationBaseMs: v.optional(v.number()),
    lastAdvancedAt: v.optional(v.number()),
    projectedCompletionAt: v.optional(v.number()),
    savedProgress: v.optional(v.record(v.string(), v.object({
      kind: v.union(v.literal("project"), v.literal("doctrine")),
      project: v.optional(v.string()),
      doctrine: v.optional(v.union(v.literal("taxItAll"), v.literal("militaryState"), v.literal("gemheartBaron"))),
      level: v.optional(v.number()),
      accumulatedBaseMs: v.number(),
      durationBaseMs: v.number(),
    }))),
